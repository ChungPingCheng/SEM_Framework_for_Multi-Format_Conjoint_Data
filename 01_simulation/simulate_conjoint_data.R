...
# Simulate data for a unified SEM framework in conjoint analysis
# Includes: rating, binary, pairwise, ranking, and categorical choice data
...
k <- 3    # number of attributes per product

# Define product attribute matrix (binary-coded attributes for 4 products)
# For example: Flavor (chocolate=0, vanilla=1), Packaging (simple=0, deluxe=1)
T <- matrix(c(
...
colnames(T) <- c("Intercept","Flavor", "Packaging")

# Latent utilities: eta_i ~ MVN(mu, Psi)
mu <- c(3, 0.5, 0.5)
corr <- matrix(c(1, 0.3, 0.3, 0.3, 1,0.3,0.3,0.3,1), nrow = 3)
variances <- c(1, 1, 2.25)
std_dev_mat <- diag(sqrt(variances))
Psi <- std_dev_mat %*% corr %*% std_dev_mat


eta <- MASS::mvrnorm(N, mu = mu, Sigma = Psi)
sigma <- 1
...
# Recreate the choice_sets (as in simulation)
